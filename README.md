...
└── src
    ...
    ├── page-style.cjs                your custom JS page script
    └── site-main.ts                your domain and website config
```
...
2. Edit `site-main.ts` and set all the necessary options: domain, metadata, slugs, subdomain redirects, etc. All settings should be self explanatory, I hope 😊

```ts filename="src/site-main.ts"
...
> [!IMPORTANT]
> You need to run deploy every time you update `page-style.cjs` or `site-main.ts`.
...
The file `src/page-style.cjs` contains an example of a page script that you can run on your Notion pages.
...
So just add your JS magic to `page-style.cjs`, run deploy and everything else will happen automagically 😎
